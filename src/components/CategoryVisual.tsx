...
/**
 * Same responsive set as {@link bundledSrcSet} but pointing at the WebP
 * siblings generated by scripts/optimize-images.mjs — roughly half the
 * bytes of the jpg at the same dimensions. Used as the <picture> `<source>`,
 * with the jpg `<img>` staying as the universally-supported fallback.
 */
export function bundledWebpSrcSet(src?: string): string | undefined {
  const jpgSet = bundledSrcSet(src);
  return jpgSet?.replace(/\.jpg/g, ".webp");
}
  ...
  if (src) {
    return (
      ...
      >
        ...
        >
          <picture>
            <source type="image/webp" srcSet={bundledWebpSrcSet(src)} sizes={sizes} />
            <img
              src={src}
